...
export default function UpdateClient({ client }: Props) {
  ...
  const submitData = async (e: React.SyntheticEvent) => {
    e.preventDefault();
    try {
      await fetch(`/api/case/${client.ClientID}`, {
        method: "PUT",
        ...
        <main className="flex min-h-screen flex-col">
          ...
          >
            <h1 className="text-center font-bold tracking-tight text-white sm:text-[3rem]">
              <span className="text-agila">Update</span> Client{" "}
              {client.FirstName}
            </h1>
            <div className="grid grid-cols-2 gap-3 rounded-lg bg-white p-4 pb-5">
              {Object.entries(client).map(([k, v], index) => (
                <div key={index} className="flex flex-col">
                  ...
                  <input
                    ...
                    disabled={["CaseNum", "ContractID", "ClientID"].includes(k)}
                    placeholder={k}
